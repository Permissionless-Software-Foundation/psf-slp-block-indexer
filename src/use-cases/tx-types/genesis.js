...
class Genesis {
  constructor (localConfig = {}) {
    ...
    if (!localConfig.adapters) {
      throw new Error('Adapters are required for the genesis.js use case library.')
    }
    ...
    this.addTokenToDB = this.addTokenToDB.bind(this)
    this.addReceiverAddress = this.addReceiverAddress.bind(this)
    this.addBatonAddress = this.addBatonAddress.bind(this)
    this.updateBalanceFromGenesis = this.updateBalanceFromGenesis.bind(this)
  }
  ...
  async addTokenToDB (data) {
    try {
      ...
      await this.adapters.tokenDb.createToken(slpData.tokenId, token)
  ...
  async addReceiverAddress (data) {
    try {
      ...
      await this.adapters.utxoDb.createUtxo(`${utxo.txid}:${utxo.vout}`, utxo)
