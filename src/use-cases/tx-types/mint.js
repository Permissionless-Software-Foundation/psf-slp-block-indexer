...
class Mint {
  constructor (localConfig = {}) {
    ...
    if (!localConfig.adapters) {
      throw new Error('Adapters are required for the mint.js use case library.')
    }
  ...
  async processTx (data) {
    try {
      ...
      if (!isValid) {
        ...
        await this.adapters.txDb.createTx(data.txData.txid, data.txData)
  ...
  async updateTokenStats (data) {
    try {
      ...
      const tokenStats = await this.adapters.tokenDb.getToken(slpData.tokenId)
      ...
      await this.adapters.tokenDb.createToken(slpData.tokenId, tokenStats)
