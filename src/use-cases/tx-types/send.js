...
import IndexerUtils from '../utils.js'
...
class Send {
  constructor (localConfig = {}) {
    ...
    if (!localConfig.adapters) {
      throw new Error('Adapters are required for the send.js use case library.')
    }
    ...
    this.updateTokenStats = this.updateTokenStats.bind(this)
    this.updateOutputAddr = this.updateOutputAddr.bind(this)
    this.addUtxoToOutputAddr = this.addUtxoToOutputAddr.bind(this)
    this.subtractTokensFromInputAddr = this.subtractTokensFromInputAddr.bind(this)
    this.subtractBalanceFromSend = this.subtractBalanceFromSend.bind(this)
    this.addTokensFromOutput = this.addTokensFromOutput.bind(this)
  }
  ...
  async updateTokenStats (data, diffBN, spentBN, sentBN) {
    try {
      ...
      const tokenData = await this.adapters.tokenDb.getToken(tokenId)
      ...
      await this.adapters.tokenDb.createToken(tokenId, tokenData)
  ...
  async reverseAddTokenFromOutput (data) {
    try {
      ...
      for (let i = 0; i < slpData.amounts.length; i++) {
        ...
        const addrData = await this.adapters.addrDb.getAddr(recvrAddr)
  ...
  async updateOutputAddr (data, voutIndex) {
    try {
      ...
      try {
        ...
        addr = await this.adapters.addrDb.getAddr(recvrAddr)
      } catch (err) {
        // New address.
      ...
      await this.adapters.addrDb.createAddr(recvrAddr, addr)

      // Add the utxo to the utxo database
      await this.adapters.utxoDb.createUtxo(`${newUtxo.txid}:${newUtxo.vout}`, newUtxo)
  ...
  async subtractTokensFromInputAddr (data) {
    try {
      ...
      for (let i = 0; i < txData.vin.length; i++) {
        ...
        try {
          const checkTxData = await this.adapters.txDb.getTx(thisVin.txid)
        ...
        const addrData = await this.adapters.addrDb.getAddr(thisVin.address)
        // console.log(`data: ${JSON.stringify(data, null, 2)}`)
        // console.log(`addrData: ${JSON.stringify(addrData, null, 2)}`)
        // process.exit(0)
